...
export default function RecordPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setPrevRecords(recRes.data.slice(0, 3));
      // サイクルタイムのデフォルトをncProgram.machiningTimeから設定
      if (d.machiningTime) {
        setCycleM(d.machiningTime);
        setCycleS(0);
      }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`flex-1 overflow-y-auto p-5 transition-opacity ${
          ...
          <div className="max-w-xl space-y-5">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-4">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">時間入力</label>
            ...
            {/* 加工個数 */}
            <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">加工個数</label>
        ...
        <div className="w-72 shrink-0 bg-white border-l border-slate-200 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">前回の作業記録</h3>
            {prevRecords.length === 0 ? (
              <p className="text-xs text-slate-400">記録なし</p>
            ) : (
              <div className="space-y-3">
                {prevRecords.map(r => (
                  <div key={r.id} className="bg-slate-50 rounded-lg p-3 border border-slate-200 text-xs space-y-1">
                    ...
                    {r.note && (
                      <div className="text-slate-400 line-clamp-2 pt-1 border-t border-slate-100">{r.note}</div>
                    )}
                  </div>
                ))}
              </div>
