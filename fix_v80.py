            ...
            <div className="flex items-center gap-1 justify-between">
              ...
              {dirtyCount > 0 && (
                <button onClick={saveAll} disabled={saving}
                  className="px-2 py-0.5 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-bold rounded disabled:opacity-50 whitespace-nowrap">
                  💾{dirtyCount}件保存
                </button>
              )}
            </div>
                ...
                <table className="w-full text-xs table-fixed">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    ...
                    {fields.length === 0 && !loading && (
                      <tr><td colSpan={6} className="text-center py-8 text-slate-400 text-xs">フィールドなし</td></tr>
                    )}
                  </tbody>
                </table>
              )}
        ...
        <div className="flex-1 overflow-auto bg-slate-300 flex items-start justify-center p-4">
          {!pdfBytes ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-4 select-none">
              ...
              <p className="text-xs text-center text-slate-400">フィールドをドラッグして位置調整できます</p>
            </div>
          ) : (
            <div className="relative shadow-2xl" style={{ width: canvasSize.w, height: canvasSize.h }}>
              ...
              <svg className="absolute top-0 left-0"
                ...
                {activeFields.map(f => {
                  ...
                  const isSel = selId === f.id;
                  return (
                    <g key={f.id} style={{ cursor: "grab" }} onMouseDown={e => handleSvgMouseDown(e, f.id, ex, ey)}>
                      <rect x={ex - 1} y={svgY - 1} width={Math.max(30, f.label.length * es * 0.55)} height={es + 4} rx={1}
