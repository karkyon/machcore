  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50">
      <header className="bg-slate-800 text-white px-5 py-3 flex items-center gap-3 shrink-0">
        ...
        <span className="text-base font-medium text-white">{t("search.mcSystemTitle", "MC マシニング管理システム")}</span>
        <span className="ml-auto flex items-center gap-2"><button onClick={() => router.push("/mc")} className="text-xs bg-slate-600 hover:bg-slate-500 text-white font-bold px-3 py-1.5 rounded-lg transition-colors">{t("search.backToDashboard", "← ダッシュボードへ")}</button>
            <button onClick={() => router.push("/mc/new")} className="text-xs bg-teal-500 hover:bg-teal-400 text-white font-bold px-3 py-1.5 rounded-lg transition-colors">{t("search.newRegister", "＋ 新規登録")}</button><span className="text-[10px] text-slate-400 bg-slate-700 px-2 py-0.5 rounded">{t("search.noAuthRequired", "認証不要")}</span></span>
      </header>
      <div className="flex flex-1 min-h-0">
        <aside className="w-[240px] shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-y-auto">
          <div className="p-4 space-y-2">
            <h2 className="text-sm font-bold text-slate-700">{t("search.mcPartSearch", "MC 部品検索")}</h2>
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide pt-1">{t("search.idDirect", "ID 直接指定")}</div>
            <div>
              <label className="text-sm font-bold text-slate-700 block mb-1">{t("search.mcIdLabel", "MC ID")} <span className="text-[10px] text-slate-400 font-normal">{t("search.legacyMcId", "(旧MCID)")}</span></label>
              <input type="number" value={mcIdInput} onChange={e => setMcIdInput(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} placeholder="1792" className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label className="text-sm font-bold text-slate-700 block mb-1">{t("search.machiningIdLabel", "加工ID")}</label>
              <input type="number" value={machiningIdInput} onChange={e => setMachiningIdInput(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} placeholder="7874" className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label className="text-sm font-bold text-slate-700 block mb-1">{t("search.partIdLabel", "部品ID")}</label>
              <input type="number" value={partIdInput} onChange={e => setPartIdInput(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} placeholder="3807" className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
            ...
            <div className="border-t border-slate-100 pt-2">
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <input type="text" value={nameInput} onChange={e => setNameInput(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} placeholder={t("search.namePlaceholder", "部品名称の一部")} className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={machineInput} onChange={e => setMachineInput(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} placeholder="MC10" className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
              </div>
            </div>
            <button onClick={handleSearch} disabled={loading} className="w-full bg-teal-600 hover:bg-teal-700 disabled:bg-slate-300 text-white py-2 rounded-lg text-sm font-bold transition-colors mt-1">{loading ? t("search.searching", "検索中...") : t("search.searchButton", "● 検索")}</button>
