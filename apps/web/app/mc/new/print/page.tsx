...
function McNewPrintInner() {
  ...
  const handlePreview = async () => {
    if (!token || !pending) { setPrintError(tr("mcNewPrintPage.authOrInfoMissing","認証または情報が不足しています")); return; }
    ...
    try {
      ...
      showToast(tr("mcNewPrintPage.previewOpenedNoRecordMsg2","📄 プレビューを開きました（DBに記録されません）"));
    } catch (e: any) {
      setPrintError(e.message ?? tr("mcNewPrintPage.previewGenerateFailed","プレビュー生成に失敗しました"));
    } finally {
      setPreviewing(false);
    }
  };
  ...
  const handleDirectPrint = async () => {
    ...
    try {
      ...
      setPrintError(e.message ?? tr("mcNewPrintPage.printFailedMsg3","印刷に失敗しました"));
    } finally {
      setDirectPrinting(false);
    }
  };

  if (!pending) return <div className="h-screen flex items-center justify-center text-slate-400">{tr("mcNewPrintPage.loadingEllipsis7","読み込み中…")}</div>;
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50">
        ...
        <div className="ml-auto flex items-center gap-3">
          {isAuthenticated && operator ? (
            <span className="text-xs bg-red-600 text-white px-3 py-1 rounded-full font-bold animate-pulse">
              {tr("mcNewPrintPage.workingStatus4", "作業中: {name} {elapsed}").replace("{name}", operator.name).replace("{elapsed}", fmtElapsed(elapsed))}
            </span>
          ) : (
            <button onClick={() => setAuthOpen(true)}
              className="text-xs bg-amber-500 hover:bg-amber-400 text-white px-3 py-1.5 rounded-lg font-bold transition-colors">
              {tr("mcNewPrintPage.authRequiredButton3", "🔒 要認証 — クリックして認証")}
            </button>
          )}
        </div>
      </header>
      ...
      <div className="bg-white border-b border-slate-200 px-5 py-2 flex items-center gap-4 shrink-0 flex-wrap">
        ...
        <span className="text-slate-400 text-xs ml-2">{tr("mcNewPrintPage.machiningIdTempPrefix", "加工ID（仮）: ")}<span className="font-mono font-bold text-teal-700">{pending.machining_id}</span></span>
        <span className="ml-1 text-[10px] text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">{tr("mcNewPrintPage.machiningIdConfirmNote", "印刷確定時に加工IDが確定します")}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 flex items-center justify-center">
        ...
        {!isAuthenticated ? (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-10 max-w-md w-full text-center">
            ...
            <h2 className="text-slate-700 font-bold text-lg mb-2">{tr("mcNewPrintPage.issueTitleNew2", "段取シート発行（新規）")}</h2>
            <p className="text-slate-400 text-sm mb-6">{tr("mcNewPrintPage.authRequiredForIssueDesc", "発行には担当者認証が必要です")}</p>
            <div className="bg-slate-50 rounded-xl p-4 mb-6 text-sm text-left space-y-1">
              <div className="flex justify-between"><span className="text-slate-500">{tr("mcNewPrintPage.drawingNoLabel2", "図番")}</span><span className="font-medium">{pending.drawing_no}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">{tr("mcNewPrintPage.partNameLabel2", "部品名")}</span><span className="font-medium">{pending.part_name}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">{tr("mcNewPrintPage.machiningIdTempLabel", "加工ID（仮）")}</span><span className="font-mono font-bold text-teal-700">{pending.machining_id}</span></div>
            </div>
            <button onClick={() => setAuthOpen(true)}
              className="w-full py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl transition-colors">
              {tr("mcNewPrintPage.startWorkButton5", "この作業を開始する")}
            </button>
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 max-w-md w-full">
            <h2 className="text-slate-700 font-bold text-base mb-5 flex items-center gap-2">
              <span className="text-teal-600">🖨</span> {tr("mcNewPrintPage.issueOptionsTitle2", "段取シート発行オプション")}
            </h2>
            <div className="space-y-3 mb-6">
              <label className="flex items-center gap-3 text-sm cursor-pointer">
                <input type="checkbox" checked={includeDrawings} onChange={e => setIncludeDrawings(e.target.checked)}
                  className="accent-teal-600 w-4 h-4" />
                <span className="text-slate-700">{tr("mcNewPrintPage.includeDrawingsLabel3", "図を含める")}</span>
              </label>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-5 text-xs text-amber-700">
              {tr("mcNewPrintPage.directPrintOnlyNote1", "⚠️ 「プリンタに直接印刷」のみ加工IDが確定し、MCデータが登録されます。")}<br/>
              {tr("mcNewPrintPage.directPrintOnlyNote2", "この画面を離脱した場合、MCデータは登録されません。")}
            </div>
            <button onClick={handlePreview} disabled={previewing || printing}
              ...
              {previewing ? tr("mcNewPrintPage.pdfGeneratingLabel2", "PDF生成中...") : tr("mcNewPrintPage.previewWatermarkButton3", "📄 プレビュー（透かし入り・記録なし）")}
            </button>
            <button onClick={handleDirectPrint} disabled={printing || previewing}
              ...
              {printing ? tr("mcNewPrintPage.registeringSendingLabel", "登録・送信中...") : tr("mcNewPrintPage.directPrintConfirmButton", "🖨 プリンタに直接印刷（加工IDを確定）")}
            </button>
            <button onClick={() => { logout(); if (typeof window !== "undefined") sessionStorage.removeItem("mc_new_pending"); router.push("/mc"); }}
              ...
              {tr("mcNewPrintPage.cancelBackToDashboardButton", "✗ キャンセル（ダッシュボードへ戻る）")}
            </button>
          </div>
        )}
      </div>
...
export default function McNewPrintPage() {
  const { t: tr } = useLanguage();
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-screen text-slate-400">{tr("mcNewPrintPage.loadingEllipsis7","読み込み中…")}</div>}>
      <McNewPrintInner />
    </Suspense>
  );
}
