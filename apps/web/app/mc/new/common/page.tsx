...
export default function McNewCommonPage() {
  ...
  const handleRegister = async () => {
    ...
    if (!selectedRow)  { setSaveError(tr("mcNewCommonPage.selectSourceRequired","供用する加工を選択してください")); return; }
    if (!selectedPart) { setSaveError(tr("mcNewCommonPage.selectTargetPartRequired","登録先の部品を選択してください")); return; }
    if (!confirm(tr("mcNewCommonPage.confirmDrawingCheck","両方の図面を見比べて、加工内容に相違ないか確認しましたか？\n\n今、手元に２枚の図面がありますか？"))) return;
    ...
    try {
      ...
      setSaveError(e?.response?.data?.message ?? e?.message ?? tr("mcNewCommonPage.registerFailedDefault3","登録失敗"));
      setSaving(false);
    }
  };
    ...
    <div className="h-screen flex flex-col bg-slate-50 overflow-hidden">
      ...
      <header className="bg-slate-800 text-white px-5 py-3 flex items-center gap-3 shrink-0">
        <button onClick={() => router.push("/mc")}
          ...
          {tr("mcNewCommonPage.backToDashboardLink7", "← ダッシュボードへ戻る")}
        </button>
        ...
        {operator && (
          <span className="ml-auto text-xs text-emerald-400">{tr("mcNewCommonPage.authenticatedLabel3", "✓ 認証済: {name}").replace("{name}", operator.name)}</span>
        )}
      </header>
        ...
        <aside className="w-[260px] shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
              {tr("mcNewCommonPage.step1SelectTargetPart", "① 登録先の部品を検索")}
            </p>
            <select value={partSearchType} onChange={e => setPartSearchType(e.target.value as any)}
              ...
              <option value="drawing_no">{tr("mcNewCommonPage.drawingNoOption3","図面番号")}</option>
              <option value="part_id">{tr("mcNewCommonPage.partIdOption3","部品ID")}</option>
              <option value="part_name">{tr("mcNewCommonPage.partNameOption3","名称")}</option>
            </select>
            <div className="flex gap-1.5">
              <input value={partSearchQ} onChange={e => setPartSearchQ(e.target.value)}
                onKeyDown={e => e.key === "Enter" && doSearchPart()}
                placeholder={tr("mcNewCommonPage.enterToSearchPlaceholder","Enterで検索")}
              ...
              <button onClick={doSearchPart} disabled={partLoading}
                ...
                {partLoading ? "…" : tr("mcNewCommonPage.searchButton8","検索")}
              </button>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {partResults.length === 0 && !partLoading && (
              <div className="p-4 text-xs text-slate-400 text-center mt-4">
                {tr("mcNewCommonPage.selectTargetPartHint", "登録先の部品を検索してください")}
              </div>
            )}
            {partResults.map(p => (
              <button key={p.id}
        ...
        <main className="flex-1 overflow-y-auto p-5">
          ...
          {!selectedPart ? (
            <div className="bg-slate-100 border-2 border-dashed border-slate-300 rounded-xl p-6 mb-5 text-sm text-slate-400 text-center">
              {tr("mcNewCommonPage.selectPartFromLeftHint3", "← 左ペインで登録先の部品を検索・選択してください")}
            </div>
          ) : (
            <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-5">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-bold text-teal-600 uppercase tracking-wider">{tr("mcNewCommonPage.targetPartLabel", "登録先の部品")}</p>
          ...
          <div className="border-t border-slate-200 pt-5">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">
              {tr("mcNewCommonPage.step2SelectSourceMachining", "② 供用する加工データを検索")}
            </h2>
            <div className="flex gap-2 mb-3">
              <select value={searchKey} onChange={e => setSearchKey(e.target.value as any)}
                ...
                <option value="machining_id">{tr("mcNewCommonPage.machiningIdOption","加工ID")}</option>
                <option value="part_id">{tr("mcNewCommonPage.partIdOption3","部品ID")}</option>
              </select>
              ...
              <button onClick={doSearchMachining} disabled={searchLoading}
                ...
                {searchLoading ? "…" : tr("mcNewCommonPage.searchButton8","検索")}
              </button>
            </div>
                ...
                {searchResults.map(row => (
                  <button key={row.mc_id} onClick={() => setSelectedRow(row)}
                    ...
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="font-mono text-[10px] text-violet-600 font-bold">{tr("mcNewCommonPage.machiningIdColonPrefix","加工ID:")}{row.machining_id}</span>
            ...
            {selectedRow && (
              <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-bold text-violet-600 uppercase tracking-wider">{tr("mcNewCommonPage.targetMachiningDataLabel", "供用する加工データ")}</p>
                  <button onClick={() => setSelectedRow(null)} className="text-xs text-slate-400 hover:text-red-500">✕</button>
                </div>
                <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
                  <div>
                    <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.machiningIdLabel4", "加工ID")}</span>
                    <span className="font-mono font-bold text-violet-700 text-lg">{selectedRow.machining_id}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.mcIdLabel2", "MCID")}</span>
                    <span className="font-mono font-bold text-blue-600">{selectedRow.legacy_mcid ?? "—"}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.drawingNoLabel5", "図面番号")}</span>
                    <span className="font-mono font-bold text-slate-800">{selectedRow.drawing_no}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.nameLabel4", "名称")}</span>
                    <span className="text-slate-700">{selectedRow.part_name}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.machineLabel9", "機械")}</span>
                    <span className="text-slate-600">{selectedRow.machine_code ?? "—"}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.versionLabel4", "バージョン")}</span>
                    <span className="font-mono text-slate-600">{selectedRow.version}</span>
                  </div>
                  {selectedRow.common_part_code && (
                    <div className="col-span-2">
                      <span className="text-[10px] text-slate-400 block">{tr("mcNewCommonPage.commonPartCodeLabel3", "共通部品コード")}</span>
          ...
          <div className="flex gap-3 max-w-xl mt-2">
            {!isAuthenticated ? (
              <button onClick={() => setAuthOpen(true)}
                ...
                {tr("mcNewCommonPage.authFirstButton3", "🔒 先に認証してください")}
              </button>
            ) : (
              <button onClick={handleRegister} disabled={!canSubmit}
                ...
                {saving ? tr("mcNewCommonPage.registeringLabel4", "登録中…") : tr("mcNewCommonPage.commonRegisterButton2", "📋 共通登録（供用）する")}
              </button>
            )}
            <button onClick={() => router.push("/mc")}
              ...
              {tr("mcNewCommonPage.cancelButton8", "キャンセル")}
            </button>
          </div>

          <div className="mt-5 max-w-xl bg-slate-100 rounded-xl p-3 text-xs text-slate-500 space-y-0.5">
            <p className="font-bold text-slate-600 mb-1">{tr("mcNewCommonPage.aboutCommonRegisterTitle", "共通加工登録について")}</p>
            <p>{tr("mcNewCommonPage.commonStep1", "① 左ペインで「登録先の部品」を検索・選択")}</p>
            <p>{tr("mcNewCommonPage.commonStep2", "② 右ペインで「供用したい加工データ」を検索・選択")}</p>
            <p>{tr("mcNewCommonPage.commonStep3", "③ 認証後「図面確認」を経て供用登録を実行")}</p>
            <p>{tr("mcNewCommonPage.commonStep4Mc", "④ 同一の加工ID（ツーリング・WO・IP含む）を別部品でも使用可能になります")}</p>
          </div>
        </main>
      </div>
