      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-[260px] shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            ...
            <select value={searchKey} onChange={e => setSearchKey(e.target.value as any)}
              className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-xs font-bold mb-2 focus:outline-none focus:ring-2 focus:ring-violet-400">
            ...
            <div className="flex gap-1.5">
              <input value={searchQ} onChange={e => setSearchQ(e.target.value)}
                ...
                className="flex-1 border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-violet-400" />
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {searchResults.map(row => (
              <button key={row.mc_id}
                onClick={() => setSelectedRow(row)}
                className={`w-full text-left px-3 py-2.5 border-b border-slate-100 hover:bg-violet-50 transition-colors
        ...
        <main className="flex-1 overflow-y-auto p-5">
          ...
          <div className="border-t border-slate-200 pt-5">
            ...
            <div className="flex gap-2 mb-3">
              ...
              <input value={partSearchQ} onChange={e => setPartSearchQ(e.target.value)}
                ...
                placeholder="Enterで検索"
                className="flex-1 max-w-xs border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
              <button onClick={doSearchPart} disabled={partLoading}
                className="px-4 py-1.5 bg-teal-600 text-white text-xs font-bold rounded-lg hover:bg-teal-700 disabled:opacity-50">
                {partLoading ? "…" : "検索"}
              </button>
            </div>
            {partResults.length > 0 && (
              <div className="border border-slate-200 rounded-xl overflow-hidden mb-4 max-h-48 overflow-y-auto">
                {partResults.map(p => (
                  <button key={p.id} onClick={() => setPartResults([])||setSelectedPart(p)}
                    className={`w-full text-left px-4 py-2.5 border-b border-slate-100 hover:bg-teal-50 transition-colors
                    ...
                    <div className="text-xs text-slate-600">{p.name}</div>
            ...
            {selectedPart && (
              <div className="bg-teal-50 border border-teal-200 rounded-xl p-3 mb-4 flex items-center gap-4">
          ...
          <div className="mt-5 max-w-xl bg-slate-100 rounded-xl p-3 text-xs text-slate-500 space-y-0.5">
            ...
            <p>① 左ペインで「供用したい加工データ」を検索・選択</p>
            <p>② 右ペインで「登録先の部品」を選択</p>
