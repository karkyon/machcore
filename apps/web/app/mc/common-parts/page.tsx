...
export default function CommonPartsPage() {
  ...
  const doUnregister = async (item: McCommonSearchResult) => {
    if (!isAuthenticated) { setAuthOpen(true); return; }
    if (!token) return;
    if (!confirm(`MCID:${item.mcProgramId} (${item.drawingNo}) の共通登録を解除しますか？`)) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] flex flex-col">
      <div className="bg-[#1b2a41] px-6 py-3 flex items-center gap-4 shrink-0">
        <button onClick={() => router.push("/mc")} className="text-blue-300 hover:text-white text-sm font-semibold">← MC</button>
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-3">
          {([["図面番号", drawingNo, setDrawingNo], ["名称", name, setName],
             ...
             ["MCID", mcId, setMcId], ["加工ID", machiningId, setMachiningId]] as [string, string, (v: string) => void][])
            .map(([lbl, val, set]) => (
              <div key={lbl}>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <table className="w-full text-xs border-collapse">
              <thead className="bg-[#1b2a41] text-white sticky top-0">
                <tr>
                  {["図面番号","名称","主機種型式","部品ID","MCID","加工ID","Ver","共通CD","G件数","状態","操作"].map(h => (
