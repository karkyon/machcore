...
function McPrintPageInner() {
  ...
  const validateRepeat = (): string | null => {
    ...
    if (!repeatConfirmed) return tr("mcPrintPage.confirmBeforeIssueRequired","発行前の確認を完了してください");
    if (repeatPurpose !== 'reference' && (!repeatQty || repeatQty < 1)) return tr("mcPrintPage.inputWorkQtyRequired","ワーク数を入力してください");
    if (repeatPurpose !== 'reference' && !repeatMachineId) return tr("mcPrintPage.selectMachineRequired","使用機械を選択してください");
    return null;
  };
  ...
  const handlePrint = async () => {
    if (!token) { setPrintError(tr("mcPrintPage.authRequired2","認証が必要です")); return; }
    ...
    try {
      ...
      showToast(tr("mcPrintPage.previewOpenedMsg","📄 プレビューを開きました（DBに記録されません）"));
    } catch (e: any) {
      setPrintError(e.message ?? tr("mcPrintPage.pdfGenerateFailed","PDF生成に失敗しました"));
    } finally {
      setPrinting(false);
    }
  };
  ...
  const handleDirectPrint = async () => {
    ...
    try {
      ...
      setPrintError(e.message ?? tr("mcPrintPage.printFailed","印刷に失敗しました"));
    } finally {
      setDirectPrinting(false);
    }
  };

  if (!nc) return <div className="h-screen flex items-center justify-center text-slate-400">{tr("mcPrintPage.loadingEllipsis3","読み込み中…")}</div>;
    ...
    if (repeatConfirmed) return (
      <div className="mb-4 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm flex items-center justify-between">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="font-bold text-emerald-700">{tr("mcPrintPage.confirmCompleteMsg", "✅ 発行前の確認完了")}</span>
          <span className="text-slate-600">{tr("mcPrintPage.purposeLabel", "用途: ")}<span className="font-bold">{repeatPurpose === 'setup' ? tr("mcPrintPage.purposeSetup", "段取") : repeatPurpose === 'reference' ? tr("mcPrintPage.purposeReference", "参考資料") : tr("mcPrintPage.purposeContinuous", "連続使用")}</span></span>
          {repeatPurpose !== 'reference' && <span className="text-slate-600">{tr("mcPrintPage.quantityWLabel2", "W数: ")}<span className="font-bold">{repeatQty}</span></span>}
          {repeatPurpose !== 'reference' && repeatMachineId && (
            <span className="text-slate-600">{tr("mcPrintPage.machineLabel4", "機械: ")}<span className="font-bold">{machines.find(m => m.id === repeatMachineId)?.machineCode ?? '—'}</span></span>
          )}
        </div>
        <button onClick={() => setRepeatConfirmed(false)} className="text-xs text-slate-500 underline ml-4 shrink-0">{tr("mcPrintPage.changeButton", "変更")}</button>
      </div>
    );
    return (
      <div className="mb-4 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 space-y-3">
        <div className="text-sm font-bold text-amber-800">{tr("mcPrintPage.confirmBeforeIssueTitle", "⚠️ 発行前の確認")}</div>
        {/* 用途 */}
        <div>
          <label className="text-xs font-bold text-slate-600 mb-1 block">{tr("mcPrintPage.purposeLabel2", "用途")}</label>
          <div className="flex gap-3 flex-wrap">
            {([['setup', tr("mcPrintPage.purposeSetup", "段取")],['reference', tr("mcPrintPage.purposeReference", "参考資料")],['continuous', tr("mcPrintPage.purposeContinuous", "連続使用")]] as const).map(([val,label]) => (
        ...
        {repeatPurpose !== 'reference' && (
          <div>
            <label className="text-xs font-bold text-slate-600 mb-1 block">{tr("mcPrintPage.usedMachineLabel", "使用機械 ")}<span className="text-red-500">*</span></label>
            <select value={repeatMachineId ?? ''}
              ...
              <option value="">{tr("mcPrintPage.selectPlaceholder2", "— 選択してください —")}</option>
        ...
        <div>
          <button
            onClick={() => {
              if (repeatPurpose !== 'reference' && (!repeatQty || repeatQty < 1)) { setPrintError(tr("mcPrintPage.inputWorkQtyRequired","ワーク数を入力してください")); return; }
              if (repeatPurpose !== 'reference' && !repeatMachineId) { setPrintError(tr("mcPrintPage.selectMachineRequired","使用機械を選択してください")); return; }
              setPrintError(null);
          ...
          >{tr("mcPrintPage.confirmCompleteButton", "確認完了")}</button>
        </div>
      </div>
    );
  })();
    ...
    <div className="h-screen flex flex-col bg-slate-50">
      <header className="bg-slate-800 text-white px-5 py-2.5 flex items-center gap-3 shrink-0">
        ...
        >
          ...
          {tr("mcPrintPage.mcDetailTabLink", "MC詳細")}
        </button>
        ...
        <span className="ml-auto">
          {isAuthenticated && operator && (
            <span className="text-[11px] bg-red-600 text-white px-2 py-0.5 rounded font-bold animate-pulse">
              {tr("mcPrintPage.workingStatus", "作業中: {name} {elapsed}").replace("{name}", operator.name).replace("{elapsed}", fmtElapsed(elapsed))}
            </span>
          )}
          {!isAuthenticated && (
            <span className="text-[11px] bg-slate-600 text-white px-2 py-0.5 rounded">{tr("mcPrintPage.authWaiting", "🔒 認証待ち")}</span>
          )}
        </span>
      </header>
      ...
      <div className="bg-white border-b border-slate-200 px-5 py-3 shrink-0">
        ...
        <div className="flex items-center gap-3 text-[13px] text-slate-500 font-mono font-medium">
          {(d as any)?.mcProcessNo != null && <span className="font-bold text-teal-700 text-sm">{tr("mcPrintPage.processNoLabel4", "工程No: {n}").replace("{n}", String((d as any).mcProcessNo))}</span>}
          <span className="text-slate-400">|</span>
          <span>{tr("mcPrintPage.mcIdPrefix2", "MCID: ")}<span className="text-slate-700">{d.legacyMcid ?? "—"}</span></span>
          <span className="text-slate-400">|</span>
          <span>{tr("mcPrintPage.machiningIdPrefix2", "加工ID: ")}<span className="text-slate-700">{d.machiningId}</span></span>
          {d.part.partId && <><span className="text-slate-400">|</span><span>{tr("mcPrintPage.partIdPrefix2", "部品ID: ")}<span className="text-slate-700">{d.part.partId}</span></span></>}
        </div>
      </div>
      ...
      <nav className="bg-white border-b border-[#d0d8e4] px-4 flex gap-1.5 items-end shrink-0 pt-1.5">
        <button onClick={() => !isNewEntry && router.push(`/mc/${mcId}`)}
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>{tr("mcPrintPage.mcDetailTabLink", "MC詳細")}
        </button>
        <button onClick={() => !isNewEntry && router.push(`/mc/${mcId}/edit`)}
          className={"px-4 py-1.5 text-[12px] font-semibold flex items-center gap-1.5 rounded-t-md border border-b-0 transition-colors " + (isNewEntry ? "border-slate-200 bg-slate-100 text-slate-300 cursor-not-allowed pointer-events-none opacity-40" : "border-[#c4cfdb] bg-white text-[#4a5568] hover:bg-[#eef3f8] hover:text-[#1b2a41]")}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>{tr("mcPrintPage.editRegisterTabLink", "変更・登録")}
        </button>
        <button onClick={() => router.push(`/mc/${mcId}/print`)}
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>{tr("mcPrintPage.setupSheetTabLink", "段取シート")}
        </button>
        <button onClick={() => !isNewEntry && router.push(`/mc/${mcId}/record`)}
          className={"px-4 py-1.5 text-[12px] font-semibold flex items-center gap-1.5 rounded-t-md border border-b-0 transition-colors " + (isNewEntry ? "border-slate-200 bg-slate-100 text-slate-300 cursor-not-allowed pointer-events-none opacity-40" : "border-[#c4cfdb] bg-white text-[#4a5568] hover:bg-[#eef3f8] hover:text-[#1b2a41]")}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>{tr("mcPrintPage.workRecordTabLink", "作業記録")}
        </button>
      </nav>

      {isAuthenticated && operator && (
        <div className="bg-red-600 text-white px-5 py-1.5 flex items-center gap-3 text-xs shrink-0">
          <span className="font-bold">{tr("mcPrintPage.workingBarLabel", "⚡ 作業中:")}</span><span>{operator.name}</span>
          <span className="font-mono bg-red-700 px-2 py-0.5 rounded">{fmtElapsed(elapsed)}</span>
          <span className="text-red-300">{tr("mcPrintPage.setupSheetIssueBar", "段取シート発行")}</span>
        </div>
      )}
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 max-w-lg w-full">
              <div className="text-center mb-6">
                ...
                <h2 className="text-slate-700 font-bold text-lg mb-2">{tr("mcPrintPage.setupSheetIssueTitle", "段取シート発行")}</h2>
                <p className="text-slate-400 text-sm">{tr("mcPrintPage.authRequiredForIssue", "発行には担当者認証が必要です")}</p>
              </div>
              <div className="border border-slate-200 rounded-xl overflow-hidden text-sm mb-6">
                <div className="grid grid-cols-3 divide-x divide-slate-200 border-b border-slate-200">
                  <div className="p-2.5 text-center"><div className="text-slate-400 text-xs mb-1">{tr("mcPrintPage.mcProcessNoLabel3", "MC工程No")}</div><div className="font-bold text-teal-700">{(d as any).mcProcessNo ?? "—"}</div></div>
                  <div className="p-2.5 text-center"><div className="text-slate-400 text-xs mb-1">{tr("mcPrintPage.versionLabel3", "バージョン")}</div><div className="font-mono font-bold">{(d as any).version ?? "—"}</div></div>
                  <div className="p-2.5 text-center"><div className="text-slate-400 text-xs mb-1">{tr("mcPrintPage.machineLabel3", "機械")}</div><div className="font-bold">{d.machine?.machineCode ?? "—"}</div></div>
                </div>
                <div className="grid grid-cols-3 divide-x divide-slate-200 border-b border-slate-200">
                ...
                <div className="p-2.5 flex items-center justify-center gap-2 flex-wrap">
                  ...
                  <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full border ${(d as any).hasIndexProgram ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-slate-50 text-slate-400 border-slate-200"}`}>IP {(d as any).hasIndexProgram ? tr("mcPrintPage.hasShort2", "有") : tr("mcPrintPage.noneShort3", "無")}</span>
                  <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full border ${(d as any).hasWorkOffset ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-slate-50 text-slate-400 border-slate-200"}`}>WD {(d as any).hasWorkOffset ? tr("mcPrintPage.hasShort2", "有") : tr("mcPrintPage.noneShort3", "無")}</span>
                </div>
              </div>
              <button onClick={() => setAuthOpen(true)}
                className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 rounded-xl text-sm">
                {tr("mcPrintPage.startWorkButton2", "この作業を開始する")}
              </button>
            </div>
          </div>
        ) : (
          <div className="max-w-xl mx-auto">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="bg-teal-600 px-5 py-3 text-white">
                <h2 className="font-bold">{tr("mcPrintPage.issueOptionsTitle", "段取シート発行オプション")}</h2>
              </div>
              <div className="p-5 pb-2 space-y-3">
                <label className="flex items-center gap-3 text-sm cursor-pointer">
                  ...
                  <span className="text-slate-700">{tr("mcPrintPage.includeDrawingsLabel", "図を含める")}</span>
                </label>
                {!isNew && (
                  <>
                    {([
                      [tr("mcPrintPage.includeToolingLabel","ツーリングリストを含める"), includeTooling,       setIncludeTooling],
                      [tr("mcPrintPage.includeClampLabel","クランプ情報を含める"),     includeClamp,         setIncludeClamp],
                      [tr("mcPrintPage.includeWorkOffsetsLabel","ワークオフセットを含める"), includeWorkOffsets,   setIncludeWorkOffsets],
                      [tr("mcPrintPage.includeIndexProgramsLabel","インデックスプログラムを含める"), includeIndexPrograms, setIncludeIndexPrograms],
              ...
              <div className="px-5 py-4 pb-6 flex flex-col gap-4 border-t border-slate-100 mt-2">
                <button onClick={() => checkSpAndPrint("preview")} disabled={printing}
                  ...
                  {printing ? tr("mcPrintPage.pdfGenerating", "PDF生成中...") : isNew ? tr("mcPrintPage.previewWatermarkButton", "📄 プレビュー（透かし入り・記録なし）") : tr("mcPrintPage.previewBrowserButton", "📄 PDFプレビュー（ブラウザで開く）")}
                </button>
                <button onClick={() => checkSpAndPrint("direct")} disabled={directPrinting}
                  ...
                  {directPrinting ? tr("mcPrintPage.sending", "送信中...") : tr("mcPrintPage.directPrintButton", "🖨 プリンタに直接印刷")}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden">
            <div className="bg-red-600 px-5 py-4 flex items-center gap-3 shrink-0">
              ...
              <div>
                <p className="text-white font-bold text-base">{tr("mcPrintPage.specialSheetTitle2", "スペシャル段取シート")}</p>
                <p className="text-red-100 text-xs">{tr("mcPrintPage.specialSheetDesc2", "過去にクレーム・トラブル実績のある製品です")}</p>
              </div>
            </div>
                ...
                <div key={s.id} className="border border-red-200 rounded-xl bg-red-50 p-4">
                  ...
                  {s.keyword && (
                    <div className="text-[11px] text-red-600 mb-2">
                      {tr("mcPrintPage.keywordPrefix", "🔑 キーワード: ")}<span className="font-bold font-mono">{s.keyword}</span>
                    </div>
                  )}
                  ...
                  {s.pdf_path && (
                    <button
                      ...
                      {tr("mcPrintPage.spPdfShowButton", "📄 SPシートPDF を表示（MCID/日時印字）")}
                    </button>
                  )}
                </div>
              ))}
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
                {tr("mcPrintPage.spSkipHint", "加工内容が異なる場合は「スキップ」を選択してください")}
              </div>
            </div>
            <div className="border-t border-slate-200 px-5 py-4 flex gap-3 justify-end shrink-0">
              <button
                ...
                {tr("mcPrintPage.cancelNoPrintButton", "キャンセル（印刷しない）")}
              </button>
              <button
                onClick={async () => {
                  setSpModalOpen(false);
                ...
                {tr("mcPrintPage.confirmedPrintButton", "確認した（印刷する）")}
              </button>
            </div>
          </div>
        </div>
...
export default function McPrintPage() {
  const { t: tr } = useLanguage();
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-screen text-slate-400">{tr("mcPrintPage.loadingEllipsis3","読み込み中…")}</div>}>
      <McPrintPageInner />
    </Suspense>
  );
}
