...
export default function NcDetailPage() {
  ...
  useLayoutEffect(() => {
    ...
    if (isAuthenticated && !isSessionForNc(ncId)) {
      console.warn("[NC-DETAIL] 認証セッションが別のnc_id向けのため強制ログアウト", { ncId });
      logout();
    }
  ...
  // [仮登録] NC詳細画面自体も、確定するまで直接アクセスされてもブロックする
  // (edit画面のNC詳細タブ非活性化に加え、URL直打ちに対する防御)。
  if (isProvisionalLocked) return (
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        {previewFile.mime_type !== "application/pdf" && (
                          <>
                            ...
                            <button onClick={() => {
                              const w = window.open("");
                              if (w) { w.document.write(`<style>@page{margin:0}html,body{margin:0;padding:0;height:100%}img{display:block;margin:auto;max-width:100vw;max-height:100vh;width:auto;height:auto;object-fit:contain;page-break-inside:avoid}</style><img src="/api/files/serve/${previewFile.id}" onload="window.print();window.close()">`); }
