...
export default function NcNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedPart)        { setSaveError(tr("ncNewPage.selectPartRequired2","部品を選択してください")); return; }
    if (!processL.trim())     { setSaveError(tr("ncNewPage.processLRequired","工程No(L)を入力してください（必須）")); return; }
    if (!machineId)           { setSaveError(tr("ncNewPage.machineRequired2","機械を選択してください（必須）")); return; }
    ...
    try {
      ...
      const errMsg = e?.response?.data?.message ?? e?.message ?? tr("ncNewPage.registerFailedDefault2","登録に失敗しました");
    ...
    <div className="h-screen flex flex-col bg-slate-50">
      <header className="bg-slate-800 text-white px-5 py-2.5 flex items-center gap-3 shrink-0">
        <button onClick={() => router.push("/")}
          ...
          {tr("ncNewPage.backToDashboardLink6", "ダッシュボードへ戻る")}
        </button>
        ...
        <span className="ml-auto">
          {isAuthenticated && authOperator
            ? <span className="text-[11px] bg-sky-700 text-white px-2.5 py-1 rounded font-bold">{tr("ncNewPage.authenticatedLabel2", "✓ 認証済: {name}").replace("{name}", authOperator.name)}</span>
            : <button onClick={() => setAuthOpen(true)} className="text-[11px] bg-amber-600 hover:bg-amber-500 text-white px-2.5 py-1 rounded font-bold transition-colors">{tr("ncNewPage.authRequiredButton2", "🔒 要認証 — クリックして認証")}</button>
          }
        </span>
      </header>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <aside className="w-[280px] shrink-0 bg-white border-r border-slate-200 flex flex-col">
          <div className="p-3 border-b border-slate-100 space-y-2">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wide">{tr("ncNewPage.step1SelectPart2", "① 部品を選択")}</h2>
            <div className="flex gap-1.5">
              <select value={searchType} onChange={e => setSearchType(e.target.value as any)}
                ...
                <option value="drawing_no">{tr("ncNewPage.drawingNoOption2", "図面番号")}</option>
                <option value="part_id">{tr("ncNewPage.partIdOption2", "部品ID")}</option>
                <option value="part_name">{tr("ncNewPage.partNameOption2", "名称")}</option>
              </select>
              <input value={searchQ} onChange={e => setSearchQ(e.target.value)}
                onKeyDown={e => e.key === "Enter" && handlePartSearch()}
                placeholder={tr("ncNewPage.searchWordPlaceholder2", "検索ワード")}
            ...
            <button onClick={handlePartSearch} disabled={partLoading || !searchQ.trim()}
              ...
              {partLoading ? tr("ncNewPage.searchingLabel3", "検索中…") : tr("ncNewPage.searchButton4", "検索")}
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            {parts.length === 0 && !partLoading && (
              <p className="text-xs text-slate-400 text-center mt-10">{tr("ncNewPage.noSearchResults2","検索結果がありません")}</p>
            )}
            {parts.map(p => (
              <button key={p.id} onClick={() => setSelectedPart(p)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {p.part_id && <span className="font-mono text-[10px] text-slate-400">{tr("ncNewPage.partIdPrefix5", "部品ID:")}{p.part_id}</span>}
                </div>
        ...
        <main className="flex-1 overflow-y-auto p-5">
          {selectedPart ? (
            <div className="bg-sky-50 border border-sky-200 rounded-xl p-3 mb-5 flex items-center gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  ...
                  {selectedPart.part_id && <span className="font-mono text-slate-500 font-bold text-lg">{tr("ncNewPage.partIdPrefix6", "部品ID: ")}{selectedPart.part_id}</span>}
                </div>
            ...
            <div className="bg-slate-100 border-2 border-dashed border-slate-300 rounded-xl p-4 mb-5 text-sm text-slate-400 text-center">
              {tr("ncNewPage.selectPartFromLeftHint2", "← 左ペインで部品を検索・選択してください")}
            </div>
          )}

          <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">{tr("ncNewPage.step2InputMachiningInfo2", "② 加工情報を入力")}</h2>
          <div className="grid grid-cols-2 gap-3 max-w-xl">
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">{tr("ncNewPage.machiningIdKLabel", "加工ID (K_id)")}</label>
              <div className="flex items-center gap-2 h-9">
                {nextIdLoading ? (
                  <span className="text-sm text-slate-400">{tr("ncNewPage.fetchingLabel2", "取得中...")}</span>
                ) : (
                  <span className="font-mono text-lg font-bold text-sky-700">{machiningId ?? "—"}</span>
                )}
                <span className="text-xs text-slate-400">{tr("ncNewPage.autoNumberingNote2", "（自動採番）")}</span>
              </div>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">
                {programNaming ? (programNaming.isFolder ? tr("ncNewPage.folderNameLabel2", "フォルダ名") : tr("ncNewPage.fileNameLabel3", "ファイル名")) : tr("ncNewPage.fileNameOrFolderName2", "ファイル名／フォルダ名")}
                <span className="text-slate-400 font-normal">{tr("ncNewPage.autoSetAfterMachineNote2", "（機械選択後に自動設定）")}</span>
              </label>
              <div className="flex items-center gap-2 h-9">
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">{tr("ncNewPage.processLLabel", "工程No (L) ")}<span className="text-red-500">*</span></label>
              <input type="number" value={processL} onChange={e => setProcessL(e.target.value)}
                placeholder={tr("ncNewPage.processLPlaceholder", "例: 1")}
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">{tr("ncNewPage.machineLabel8", "機械 ")}<span className="text-red-500">*</span></label>
              <select value={machineId} onChange={e => setMachineId(e.target.value)}
                ...
                <option value="">{tr("ncNewPage.notSetOption2", "-- 未設定 --")}</option>
                {machines.map(m => (
              ...
              {programNaming && (
                <div className={`mt-1.5 flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border ${programNaming.isFolder ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-sky-50 border-sky-200 text-sky-700"}`}>
                  <span>{programNaming.isFolder ? tr("ncNewPage.folderUnitBadge3", "📁 フォルダ単位") : tr("ncNewPage.singleFileBadge3", "📄 単体ファイル")}</span>
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">{tr("ncNewPage.machiningTimeMinLabel", "加工時間(分)")}</label>
          ...
          <div className="mt-4 max-w-xl grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">{tr("ncNewPage.clampAllowanceLabel3", "掴代 ")}<span className="text-slate-400 font-normal">{tr("ncNewPage.clampAllowanceMmNote", "(mm)")}</span></label>
              <input value={clampAllowance} onChange={e => setClampAllowance(e.target.value)} maxLength={50}
                placeholder={tr("ncNewPage.clampAllowancePlaceholder", "例: 専用 / 9~10")}
                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400" />
            </div>
            <div className="col-span-2">
              <label className="text-xs font-bold text-slate-700 block mb-1">{tr("ncNewPage.noteLabel6", "備考")}</label>
              <textarea value={clampNote} onChange={e => setClampNote(e.target.value)} rows={2} maxLength={2000}
                placeholder={tr("ncNewPage.notePlaceholder4", "特記事項・注意事項")}
          ...
          <div className="mt-4 max-w-xl flex gap-3">
            {!actuallyAuthenticated ? (
              <button onClick={() => setAuthOpen(true)}
                ...
                {tr("ncNewPage.authFirstButton2", "🔒 先に認証してください")}
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={saving || !canSubmit}
                ...
                {saving ? tr("ncNewPage.registeringLabel3", "登録中…") : tr("ncNewPage.provisionalRegisterButton2", "✓ 仮登録する")}
              </button>
            )}
            <button onClick={() => router.push("/nc/search")}
              ...
              {tr("ncNewPage.cancelButton7", "キャンセル")}
            </button>
          </div>

          <div className="mt-5 max-w-xl bg-slate-100 rounded-xl p-3 text-xs text-slate-500 space-y-0.5">
            <p className="font-bold text-slate-600 mb-1">{tr("ncNewPage.flowAfterRegisterTitle2", "仮登録後の流れ")}</p>
            <p>{tr("ncNewPage.flowStep1b", "① 仮登録完了 →「変更・登録」タブへ移動（この時点ではまだ確定していません）")}</p>
            <p>{tr("ncNewPage.flowStep2b", "② 変更・登録タブでツーリング・PGファイルなどを入力し「✓ 作業完了（登録）」を押すと登録が確定します")}</p>
            <p>{tr("ncNewPage.flowStep3b", "③ 確定するまで「段取シート」「作業記録」タブは非活性です")}</p>
            <p>{tr("ncNewPage.flowStep4b", "④ 確定前にこの画面を離れた場合、登録内容は破棄され加工ID(K_id)は解放されます")}</p>
            <p>{tr("ncNewPage.flowStep5b", "⑤ 承認者が承認 → ステータスが「承認済」に変わります")}</p>
          </div>
        </main>
      </div>
