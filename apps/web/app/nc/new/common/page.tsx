...
export default function NcNewCommonPage() {
  ...
  // ── 右ペイン: 供用する加工データを検索 ──
  const [searchKey,     setSearchKey]     = useState<"drawing_no"|"name"|"nc_id"|"part_id">("drawing_no");
  const [searchQ,       setSearchQ]       = useState("");
  const [searchResults, setSearchResults] = useState<NcSearchResult[]>([]);
  const [searchLoading, setSearchLoading] = useState(false);
  const [selectedRow,   setSelectedRow]   = useState<NcSearchResult | null>(null);
  ...
  const doSearchPart = useCallback(async () => {
    ...
    setPartLoading(true);
    try {
      const res = await ncApi.searchParts(partSearchType, partSearchQ.trim());
  ...
  const doSearchMachining = useCallback(async () => {
    ...
    setSelectedRow(null);
    try {
      const res = await ncApi.search(searchKey, searchQ.trim());
      const d = (res as any).data ?? res;
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {partResults.map(p => (
              <button key={p.id}
                ...
                className={`w-full text-left px-3 py-2.5 border-b border-slate-100 hover:bg-sky-50 transition-colors
                  ${selectedPart?.id === p.id ? "bg-sky-50 border-l-4 border-l-sky-500" : ""}`}>
                <div className="font-mono text-xs font-bold text-sky-700">{p.drawing_no}</div>
            ...
            {searchResults.length > 0 && (
              <div className="border border-slate-200 rounded-xl overflow-hidden mb-4 max-h-64 overflow-y-auto">
                {searchResults.map(row => (
                  <button key={row.nc_id} onClick={() => setSelectedRow(row)}
