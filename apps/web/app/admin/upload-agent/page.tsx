...
export default function UploadAgentPage() {
  ...
  const handleDeploy = async () => {
    if (!deployFile) { setDeployMsg({ ok: false, text: t("adminUploadAgent.selectExeFile", "exeファイルを選択してください") }); return; }
    if (!newVersion.trim()) { setDeployMsg({ ok: false, text: t("adminUploadAgent.enterVersion", "バージョン番号を入力してください") }); return; }
    setDeploying(true);
    setDeployMsg(null);
    try {
      ...
      if (res.ok) {
        ...
        setDeployMsg({ ok: false, text: `❌ ${json.message ?? t("adminUploadAgent.deployFailed", "デプロイ失敗")}` });
      }
    } catch (e: any) {
    ...
    <AdminLayout pathname={pathname}>
        ...
        <main className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold text-slate-800">{t("adminUploadAgent.title", "UploadAgent 配布・管理")}</h1>
            <span className="text-xs text-slate-400 font-mono">
              {t("adminUploadAgent.distributingVersion", "配布中: v{ver}").replace("{ver}", versionInfo?.version ?? "—")}
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="font-bold text-slate-700 mb-3 flex items-center gap-2">
              ...
              {t("adminUploadAgent.agentCheckTitle", "このPC の Agent 稼働確認")}
            </h2>
            <div className="flex items-center gap-4">
              <div className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm ${
                ...
                {agentStatus === "online" ? t("adminUploadAgent.online","✅ 稼働中") : agentStatus === "offline" ? t("adminUploadAgent.offline","❌ 未起動 / 未インストール") : t("adminUploadAgent.unknown","— 未確認")}
              </div>
              <button onClick={checkAgent} disabled={checking}
                ...
                {checking ? t("adminUploadAgent.checking","確認中...") : t("adminUploadAgent.checkButton","稼働確認")}
              </button>
            </div>
            <p className="text-xs text-slate-400 mt-2">{t("adminUploadAgent.checkHint", "※ このボタンはこのPCにインストールされたAgentのみ確認できます")}</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="font-bold text-slate-700 mb-1 flex items-center gap-2">
              ...
              {t("adminUploadAgent.downloadTitle", "インストーラ ダウンロード")}
            </h2>
            <p className="text-xs text-slate-500 mb-4">{t("adminUploadAgent.downloadDesc")}</p>
            {versionInfo?.exists ? (
              <a href="/api/admin/upload-agent/download"
                ...
                {t("adminUploadAgent.downloadButton", "⬇ UploadAgent_Setup_v{ver}.exe をダウンロード").replace("{ver}", versionInfo.version)}
              </a>
            ) : (
              <div className="text-sm text-slate-400 bg-slate-50 rounded-lg p-4">{t("adminUploadAgent.noInstaller")}</div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border-2 border-amber-200 p-5">
            <h2 className="font-bold text-amber-800 mb-1 flex items-center gap-2">
              ...
              {t("adminUploadAgent.versionMgmtTitle", "バージョン管理（新バージョンをデプロイ）")}
            </h2>
            <p className="text-xs text-slate-500 mb-4">
              {t("adminUploadAgent.versionMgmtDesc")}
            </p>
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <label className="text-xs font-bold text-slate-600 w-28 shrink-0">{t("adminUploadAgent.newVersionLabel", "新バージョン番号")}</label>
              ...
              <div className="flex items-center gap-3">
                <label className="text-xs font-bold text-slate-600 w-28 shrink-0">{t("adminUploadAgent.exeFileLabel", "exeファイル")}</label>
                <label className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-bold cursor-pointer transition-colors">
                  📁 {deployFile ? deployFile.name : t("adminUploadAgent.selectFile", "ファイルを選択...")}
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleDeploy} disabled={deploying || !deployFile || !newVersion.trim()}
                  ...
                  {deploying ? t("adminUploadAgent.deploying","⏳ デプロイ中...") : t("adminUploadAgent.deployButton","🚀 デプロイ（配布ファイルを更新）")}
                </button>
              </div>
              {deployMsg && (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="font-bold text-slate-700 mb-3 flex items-center gap-2">
              ...
              {t("adminUploadAgent.installStepsTitle", "クライアントPCへのインストール手順")}
            </h2>
            <ol className="space-y-2 text-sm text-slate-700">
              {[
                t("adminUploadAgent.step1", "上記「UploadAgent_Setup_vX.exe」をダウンロード"),
                t("adminUploadAgent.step2", "ダウンロードしたexeをダブルクリックで実行"),
                t("adminUploadAgent.step3", "インストール完了後、タスクトレイに 🟢 緑のMCアイコンが表示される"),
                t("adminUploadAgent.step4", "以降Windowsログイン時に自動起動（スタートアップ登録済み）"),
                t("adminUploadAgent.step5", "MachCoreでUSBファイルをアップロードすると自動でUSB元ファイルを移動"),
              ].map((step, i) => (
          ...
          {/* ── 機能概要 ── */}
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="font-bold text-slate-700 mb-3">{t("adminUploadAgent.featuresTitle", "機能概要")}</h2>
            <div className="grid grid-cols-2 gap-3 text-sm">
              {[
                [t("adminUploadAgent.featPort","待受ポート"), t("adminUploadAgent.featPortVal","localhost:57301（外部アクセス不可）")],
                [t("adminUploadAgent.featFileOp","ファイル操作"), t("adminUploadAgent.featFileOpVal","USB元ファイルを .machcore_trash/ に移動")],
                [t("adminUploadAgent.featIcon","アイコン"), t("adminUploadAgent.featIconVal","正常=🟢緑 / エラー=🔴赤 でトレイ表示")],
                [t("adminUploadAgent.featSettings","設定画面"), t("adminUploadAgent.featSettingsVal","トレイ右クリック → 設定... で開く")],
                [t("adminUploadAgent.featLog","ログ"), "%APPDATA%\MachCore\UploadAgent\agent.log"],
                [t("adminUploadAgent.featAutoStart","自動起動"), t("adminUploadAgent.featAutoStartVal","Windowsログイン時に自動起動")],
              ].map(([k, v]) => (
