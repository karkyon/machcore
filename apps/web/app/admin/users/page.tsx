...
export default function AdminUsersPage() {
  ...
  const openCreate = () => { setFCode(""); setFName(""); setFKana(""); setFPw(""); setFPw2(""); setFRole("OPERATOR"); setFError(null); setEditTarget(null); setDialogMode("create"); };
  const openEdit   = (u: AdminUserInfo) => { setFName(u.name); setFKana(u.nameKana ?? ""); setFRole(u.role); setFError(null); setEditTarget(u); setDialogMode("edit"); };
  const openPassword = (u: AdminUserInfo) => { setFPw(""); setFPw2(""); setFError(null); setEditTarget(u); setDialogMode("password"); };
  ...
  const handleCreate = async () => {
    ...
    try {
      await adminUsersApi.create({ employee_code: fCode, name: fName, name_kana: fKana||undefined, password: fPw, role: fRole }, getToken());
      showToast("ユーザを作成しました", true); closeDialog(); fetchUsers();
    } catch (e: any) { setFError(Array.isArray(e?.response?.data?.message) ? e.response.data.message.join(", ") : (e?.response?.data?.message ?? "作成に失敗しました")); }
  ...
  const handleUpdate = async () => {
    ...
    } catch (e: any) { setFError(e?.response?.data?.message ?? "更新に失敗しました"); }
    finally { setSaving(false); }
  };
  ...
  const handlePassword = async () => {
    ...
    } catch (e: any) { setFError(e?.response?.data?.message ?? "変更に失敗しました"); }
    finally { setSaving(false); }
  };
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
                    <tr>
                      ...
                      <th className="px-5 py-3 text-left w-32">社員コード</th>
                      <th className="px-5 py-3 text-left">氏名</th>
                      <th className="px-5 py-3 text-left w-24">カナ</th>
                      <th className="px-5 py-3 text-left w-24">ロール</th>
                      <th className="px-5 py-3 text-left w-16">状態</th>
                      <th className="px-5 py-3 text-right w-36">操作</th>
                    </tr>
                  </thead>
                </table>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredUsers.map(u => (
                      <tr key={u.id} className={`hover:bg-slate-50 ${!u.isActive ? "opacity-40" : ""}`}>
                        <td className="px-5 py-3 text-slate-400 w-16">{u.id}</td>
                        <td className="px-5 py-3 font-mono text-slate-700 w-32">{u.employeeCode}</td>
                        <td className="px-5 py-3 font-medium text-slate-800">{u.name}</td>
                        <td className="px-5 py-3 text-slate-500 w-24">{u.nameKana ?? "—"}</td>
                        ...
                        <td className="px-5 py-3 text-right space-x-2 w-36">
                          ...
                          <button onClick={() => handleToggleActive(u)} className={`text-xs hover:underline ${u.isActive ? "text-red-500" : "text-green-600"}`}>
                            {u.isActive ? "無効化" : "有効化"}
                          </button>
                        </td>
                      </tr>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={closeDialog} className="flex-1 px-4 py-2 rounded-lg border border-slate-300 text-slate-600 text-sm">キャンセル</button>
                <button onClick={dialogMode === "create" ? handleCreate : dialogMode === "edit" ? handleUpdate : handlePassword}
                  disabled={saving} className="flex-1 px-4 py-2 rounded-lg bg-sky-600 text-white text-sm font-bold disabled:opacity-40">
                  {saving ? "処理中..." : "保存"}
                </button>
              </div>
            </div>
