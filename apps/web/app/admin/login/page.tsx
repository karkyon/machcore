...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async () => {
    if (!employeeCode || !password) {
      setError(t("adminLogin.codeAndPasswordRequired", "社員コードとパスワードを入力してください"));
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      const msg = e?.response?.data?.message ?? t("adminLogin.loginFailed", "ログインに失敗しました");
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center">
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden">
        ...
        <div className="bg-slate-800 px-8 py-6 text-center">
          ...
          <div className="text-slate-400 text-xs mt-1">{t("adminLogin.title", "管理者ログイン")}</div>
        </div>
        ...
        <div className="px-8 py-7 space-y-5">
          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wide">
              {t("adminLogin.employeeCode", "社員コード")}
            </label>
            <input
              type="text"
              value={employeeCode}
              ...
              placeholder={t("adminLogin.codePlaceholder", "例: ADMIN001")}
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("adminLogin.passwordPlaceholder", "パスワードを入力")}
              className="w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm
                         focus:outline-none focus:ring-2 focus:ring-sky-400"
            />
          </div>
          ...
          >
            {loading ? t("adminLogin.checking", "確認中...") : t("adminLogin.loginButton", "ログイン")}
          </button>
        </div>

        <div className="border-t border-slate-100 px-8 py-4 text-center">
          <p className="text-xs text-slate-400">{t("adminLogin.adminOnlyNote", "管理者アカウントのみログイン可能です")}</p>
          <a href="/nc/search" className="text-xs text-slate-400 hover:text-slate-600 transition-colors block text-center mt-3">{t("adminLogin.backToNc", "← NC画面に戻る")}</a>
        </div>
      </div>
    </div>
  );
