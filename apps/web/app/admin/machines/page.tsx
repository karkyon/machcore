...
export default function AdminMachinesPage() {
  ...
  const fetchMachines = useCallback(async () => {
    ...
    } catch { showToast(t("adminMachines.fetchFailed", "機械一覧の取得に失敗しました"), false); }
  ...
  const handleSave = async () => {
    ...
    } catch { setFError(t("adminMachines.communicationError", "通信エラー")); }
  ...
  const handleToggle = async (m: Machine) => {
    try {
      ...
      showToast(m.isActive ? t("adminMachines.deactivated","無効化しました") : t("adminMachines.activated","有効化しました"), true); fetchMachines();
    } catch { showToast(t("adminMachines.changeFailed", "変更失敗"), false); }
  };
      ...
      {toast && <div className={`fixed top-4 right-4 z-50 px-4 py-2 rounded-lg shadow-lg text-white text-sm font-bold ${toast.ok ? "bg-green-600" : "bg-red-600"}`}>{toast.msg}</div>}
        ...
        <main className="flex-1 overflow-hidden flex flex-col p-5 gap-3">
          <div className="flex items-center justify-between shrink-0">
            <h1 className="text-xl font-bold text-slate-800">{t("adminMachines.title", "機械一覧")}</h1>
            <button onClick={openCreate} className="bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg">{t("adminMachines.addNew", "＋ 新規機械追加")}</button>
          </div>
          <div className="flex flex-wrap gap-2 bg-white p-3 rounded-xl border border-slate-200 shrink-0">
            <input type="text" value={fltName} onChange={e => setFltName(e.target.value)} placeholder={t("adminMachines.filterName", "機械名でフィルタ")}
            ...
            <select value={fltStatus} onChange={e => setFltStatus(e.target.value)}
              className="border border-slate-300 rounded-lg px-3 py-1.5 text-sm bg-white focus:ring-2 focus:ring-sky-400 focus:outline-none">
              <option value="">{t("adminMachines.statusAll", "状態: すべて")}</option><option value="active">{t("adminMachines.activeOnly", "有効のみ")}</option><option value="inactive">{t("adminMachines.inactiveOnly", "無効のみ")}</option>
            </select>
            <span className="text-xs text-slate-400 self-center">{t("adminMachines.countFraction","{a}/{b}件").replace("{a}",String(filtered.length)).replace("{b}",String(machines.length))}</span>
          </div>

          {loading ? <div className="text-center py-20 text-slate-400">{t("adminMachines.loading", "読み込み中…")}</div> : (
                ...
                <table className="w-full text-sm table-fixed">
                  ...
                  <thead>
                    <tr className="bg-slate-50 text-slate-600 text-xs uppercase">
                      ...
                      <th className="px-3 py-3 text-left cursor-pointer select-none" onClick={() => toggleSort("isActive")}>{t("adminMachines.colStatus","状態")}<SortIcon k="isActive"/></th>
                      <th className="px-3 py-3 text-right">{t("adminMachines.colAction", "操作")}</th>
                    </tr>
                  </thead>
                </table>
              </div>
                    ...
                    {filtered.map((m, i) => (
                      <tr key={m.id} className={`${!m.isActive ? "opacity-40" : ""} ${i%2===0?"bg-white":"bg-slate-50/40"}`}>
                        ...
                        <td className="px-3 py-2.5 text-xs">
                          {(m as any).pgIsFolder
                            ? <span className="text-amber-700">{t("adminMachines.folderUnit", "📁 フォルダ単位")}</span>
                            : <span className="text-teal-700">{t("adminMachines.singleFile", "📄 単体ファイル")}</span>}
                        </td>
                        <td className="px-3 py-2.5 text-slate-500 text-xs">{m.sortOrder ?? 0}</td>
                        <td className="px-3 py-2.5">
                          <span className={`text-xs font-bold ${m.isActive ? "text-green-600" : "text-slate-400"}`}>{m.isActive ? t("adminMachines.active","有効") : t("adminMachines.inactive","無効")}</span>
                        </td>
                        <td className="px-3 py-2.5">
                          <div className="flex items-center justify-end gap-1 flex-nowrap">
                            <button onClick={() => openEdit(m)} className="px-2 py-1 text-xs bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 rounded font-bold whitespace-nowrap">{t("adminMachines.edit", "編集")}</button>
                            <button onClick={() => handleToggle(m)} className={`px-2 py-1 text-xs border rounded font-bold whitespace-nowrap ${m.isActive ? "bg-red-50 hover:bg-red-100 text-red-600 border-red-200" : "bg-green-50 hover:bg-green-100 text-green-600 border-green-200"}`}>
                              {m.isActive ? t("adminMachines.deactivate","無効化") : t("adminMachines.activate","有効化")}
                            </button>
                          </div>
                        </td>
                      </tr>
                    ))}
                    {filtered.length === 0 && <tr><td colSpan={8} className="text-center py-12 text-slate-400">{t("adminMachines.noMatch", "該当する機械がありません")}</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
      ...
      {dialogMode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm">
            <h2 className="text-lg font-bold text-slate-800 mb-4">{dialogMode === "create" ? t("adminMachines.createTitle","新規機械追加") : t("adminMachines.editTitle","機械編集")}</h2>
            {fError && <div className="text-red-500 text-xs mb-3">{fError}</div>}
            <div className="space-y-3">
              <div><label className="text-xs font-bold text-slate-500 block mb-1">{t("adminMachines.codeLabel", "機械コード *")}</label>
              ...
              <div><label className="text-xs font-bold text-slate-500 block mb-1">{t("adminMachines.nameLabel", "機械名 *")}</label>
              ...
              <div><label className="text-xs font-bold text-slate-500 block mb-1">{t("adminMachines.makerLabel", "メーカー")}</label>
                <input type="text" value={fMaker} onChange={e => setFMaker(e.target.value)}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-sky-400 focus:outline-none" /></div>
              <div><label className="text-xs font-bold text-slate-500 block mb-1">{t("adminMachines.orderLabel", "順序")}</label>
                <input type="number" value={fSort} onChange={e => setFSort(e.target.value)}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-sky-400 focus:outline-none" /></div>
              <div>
                <label className="text-xs font-bold text-slate-500 block mb-1">{t("adminMachines.pgFormatLabel", "プログラムファイル形式")}</label>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setFPgIsFolder(false)}
                    ...
                    {t("adminMachines.singleFile", "📄 単体ファイル")}
                  </button>
                  <button type="button" onClick={() => setFPgIsFolder(true)}
                    ...
                    {t("adminMachines.folderUnit", "📁 フォルダ単位")}
                  </button>
                </div>
                <p className="text-[10px] text-slate-400 mt-1">{t("adminMachines.pgFormatHint")}</p>
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setDialogMode(null)} className="px-4 py-2 text-sm text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50">{t("adminMachines.cancel", "キャンセル")}</button>
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-bold disabled:opacity-50">
                {saving ? t("adminMachines.saving","保存中…") : t("adminMachines.save","保存")}
              </button>
            </div>
          </div>
        </div>
